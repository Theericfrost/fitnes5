...
import image from "../img/pushup.jpg";
...
class Pushup extends Component {
    ...
    dayClick = (event) => {
        this.setState(
            {program: pushup[`date_${event.target.innerHTML}`].con.join('-'),
                timer: pushup[`date_${event.target.innerHTML}`].res,
                week: pushup[`date_${event.target.innerHTML}`].date,
            }
        );
    }
    Back = () => {
    ...
    Screen = () => {
        ...
        return(
            <div>
                ...
                <h1 className='Form_h1'> Отжимания</h1>
