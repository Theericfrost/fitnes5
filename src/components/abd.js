import {abd} from "../list/List";
import Timer from "./timer";
import React , {Component} from "react";
import image from "../img/abd.jpg";

class Abd extends Component {
    state = {
        program : [],
        timer : null,
        week: null,
    ...
    dayClick = (event) => {
        this.setState(
            {program: abd[`date_${event.target.innerHTML}`].con.join('-'),
                timer: abd[`date_${event.target.innerHTML}`].res,
                week: abd[`date_${event.target.innerHTML}`].date,
            }
        );
    }
    Back = () => {
        ...
        if (this.state.week == 1) {
            return (<div>
                    ...
                    <div className="Otjim">
                        ...
                        <h3>Количество повторений на пресс</h3>
