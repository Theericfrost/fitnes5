import {pullup} from "../list/List";
import Timer from "./timer";
import React , {Component} from "react";
import image from "../img/pullup.jpg";
...
class Pullup extends Component {
    ...
    dayClick = (event) => {
        this.setState(
            {program: pullup[`date_${event.target.innerHTML}`].con.join('-'),
                timer: pullup[`date_${event.target.innerHTML}`].res,
                week: pullup[`date_${event.target.innerHTML}`].date,
            }
        );
    }
    Back = () => {
    ...
    Screen = () => {
        ...
        return(
            <div>
                ...
                <h1 className='Form_h1'> Подтягивания</h1>
