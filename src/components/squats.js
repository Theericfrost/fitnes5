import {squats} from "../list/List";
import Timer from "./timer";
import React , {Component} from "react";
import image from "../img/squats.jpg";
...
class Squats extends Component {
    ...
    dayClick = (event) => {
        this.setState(
            {program: squats[`date_${event.target.innerHTML}`].con.join('-'),
                timer: squats[`date_${event.target.innerHTML}`].res,
                week: squats[`date_${event.target.innerHTML}`].date,
            }
        );
    }
    Back = () => {
    ...
    Screen = () => {
        ...
        return(
            <div>
                ...
                <h1 className='Form_h1'> Приседания</h1>
